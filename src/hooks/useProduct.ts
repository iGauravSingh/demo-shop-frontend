...
import { setproducts, clearproducts, deleteProducts, updateproduct } from "../features/productSlice"
...
// 
    ...
    const deleteProductwithId = async (id:number) => {
        try {
            const response = await axios.delete(`${urllocal}/products/deleteproduct/${id}`,{
                headers: {
                  ...(adminToken ? { Authorization: `Bearer ${adminToken}` } : null),
                },
              });
