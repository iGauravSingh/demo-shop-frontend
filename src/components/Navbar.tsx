  ...
  return (
    ...
    <div className=" w-screen h-[50px] flex justify-end items-center border-b-2 px-10 font-sans">
      <ul className=" flex items-center gap-8">
        {navmenu.map((menu) => (
          <li key={menu.id} className=" text-slate-800 cursor-pointer font-semibold"><Link to={menu.link}>{menu.name}</Link></li>
        ))}
        ...
        <div className=" cursor-pointer">{user.user?.id ? <Link to="/profile"><FaRegUserCircle color="#1e293" size={20} /></Link> : <Link to="/signin"><FaSignInAlt color="#1e293" size={20} /></Link>}</div>
        {/* FaRegUserCircle */}
      </ul>
